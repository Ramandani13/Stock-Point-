...
?>	
    ...
    if(isset($_GET['kirim1'])){
        ...
        elseif (strlen($input) <= 3 AND strlen($input) >= 2)  {
            include "koneksi.php";
            $dptprogress = mysqli_query($koneksi, "SELECT * FROM tb_sp_progress WHERE pengguna = '$getusername'");
                                while($dptprogress1 = mysqli_fetch_array($dptprogress)) { 
                                    $dptorderjumlah  = $dptprogress1['order_no'];
                                    $dptlotjumlah     = $dptprogress1['lot'];
                                    $dptlocjumlah     = $dptprogress1['loc_input'];
                                }
            $dptresult = mysqli_query($koneksi, "SELECT * FROM tb_sp_result  ");
                                while($dptresult1 = mysqli_fetch_array($dptresult)) { 
                                    $dptorderresult  = $dptresult1['order_no'];
                                    $dptlotresult     = $dptresult1['lot'];
                                    $dptlocresult     = $dptresult1['loc_input'];
                                }
            $tbinput = mysqli_num_rows(mysqli_query($koneksi, "SELECT (loc_input) AS triger FROM tb_sp_result "));

            if($tbinput  == '0'){      
            $hasil1 = mysqli_query($koneksi, "UPDATE tb_sp_progress SET loc_input='$input' WHERE order_no='$dptorderjumlah' and lot='$dptlotjumlah' and pengguna = '$getusername'");
            ...
            $deleted1 = mysqli_query($koneksi, "DELETE FROM tb_sp_progress WHERE pengguna = '$getusername'");
                ...
                if($dptorderjumlah == $dptorderresult ){
                    if($dptlotjumlah == $dptlotresult ){
                        if($input == $dptlocresult){
                            $hasil1 = mysqli_query($koneksi, "UPDATE tb_sp_progress SET loc_input='$input' WHERE order_no='$dptorderjumlah' and lot='$dptlotjumlah' and pengguna = '$getusername'");
                            $movetable1 = mysqli_query($koneksi, "INSERT INTO tb_sp_result (tgl,order_no,model,lot,loc_input,case_no,casemark,jam,pengguna,case_plan) SELECT tgl,order_no,model,lot,loc_input,case_no,casemark,jam,pengguna,case_plan FROM tb_sp_progress WHERE pengguna = '$getusername'");
                            $deleted1 = mysqli_query($koneksi, "DELETE FROM tb_sp_progress WHERE pengguna = '$getusername'");
                                ...
                                include "koneksi.php";
                                        $getfas2 = mysqli_query($koneksifas, "SELECT * FROM tb_fas where casemark = '$dptorderjumlah' and lot_no = '$dptlotjumlah'");
                                        while($getfas21 = mysqli_fetch_array($getfas2)) { 
                                                $getcase_plan2  = $getfas21['case_plan'];
                                        } 
        
                                        $getact2 = mysqli_query($koneksi, "SELECT  order_no,lot,COUNT(order_no) AS act2 FROM tb_sp_result GROUP BY order_no,lot");
                                        while($user_data2 = mysqli_fetch_array($getact2)) {  
                                                echo $getactt2 = $user_data2['act2'];
                                        }

                                if($getactt2 == $getcase_plan2){
                                    $hasil1 = mysqli_query($koneksi, "UPDATE tb_sp_progress SET loc_input='$input' WHERE order_no='$dptorderjumlah' and lot='$dptlotjumlah' and pengguna = '$getusername'");
                                    $movetable1 = mysqli_query($koneksi, "INSERT INTO tb_sp_result (tgl,order_no,model,lot,loc_input,case_no,casemark,jam,pengguna,case_plan) SELECT tgl,order_no,model,lot,loc_input,case_no,casemark,jam,pengguna,case_plan FROM tb_sp_progress WHERE pengguna = '$getusername'");
                                    $deleted1 = mysqli_query($koneksi, "DELETE FROM tb_sp_progress WHERE pengguna = '$getusername'");
                                }else{
                    ...
                    $dptresult00 = mysqli_query($koneksi, "SELECT * FROM tb_sp_result  ");
                                while($dptresult100 = mysqli_fetch_array($dptresult00)) {
                                        $getlocation00 = $dptresult100['loc_input'];
                                
                        
                        // $dptresult00 = mysqli_query($koneksi, "SELECT * FROM tb_sp_result where order_no = ' $dptorderjumlah ' and lot = '$dptlotjumlah ' ");
                        // while($dptresult100 = mysqli_fetch_array($dptresult00)) { 
                        //     $dptorderresult00  = $dptresult100['order_no'];
                        //     $dptlotresult00     = $dptresult100['lot'];
                        //     $dptlocresult00     = $dptresult100['loc_input'];
                        // }
                    }
                    if($input == $getlocation00){
                        echo '<script>alert("33333...!")</script>';
                        echo '<META HTTP-EQUIV="Refresh" Content="0; URL=index.php ">';  
                                    
                    }else{
                        $hasil1 = mysqli_query($koneksi, "UPDATE tb_sp_progress SET case_no ='dani',loc_input ='$input' WHERE order_no='$dptorderjumlah' and lot='$dptlotjumlah' and pengguna = '$getusername'");
                        $movetable1 = mysqli_query($koneksi, "INSERT INTO tb_sp_result (tgl,order_no,model,lot,loc_input,case_no,casemark,jam,pengguna,case_plan) SELECT tgl,order_no,model,lot,loc_input,case_no,casemark,jam,pengguna,case_plan FROM tb_sp_progress WHERE pengguna = '$getusername'");
                        $deleted1 = mysqli_query($koneksi, "DELETE FROM tb_sp_progress WHERE pengguna = '$getusername'");
            ...
            echo '<script>alert("kenapa...!")</script>';
            echo '<META HTTP-EQUIV="Refresh" Content="3; URL=index.php ">';
        }
