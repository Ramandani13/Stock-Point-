...
<body>
		...
		<div class="main">
          ...
          <span class="align-middle" style="position:relative;top:17px;right:242px;"><img src="gambar/stoc.png" style="border-radius:5px;" width="100" height="8"></span>
        ...
        elseif (strlen($input) <= 3 AND strlen($input) >= 2)  {
            ...
            if($loc_inputdataloc1 == ''){
                ...
                while($datalocinputdata1 = $datalocinputdata->fetch_array(MYSQLI_ASSOC)) {
                    $arrayresult[]= array("order_no"=>$datalocinputdata1['order_no'],"loc_input"=>$datalocinputdata1['loc_input']);
                }
                ...
                $datalotorder = $koneksi->query("SELECT * FROM tb_sp_result WHERE order_no = '$order_nodataloc1'");
                while($datalotorder1 = $datalotorder->fetch_array(MYSQLI_ASSOC)) {
                    $arraylotorder[]= array("lot"=>$datalotorder1['lot']);
                }

                $datalotall = $koneksi->query("SELECT * FROM tb_sp_result WHERE order_no = '$order_nodataloc1' AND lot = '$lotdataloc1'");
                while($datalotall1 = $datalotall->fetch_array(MYSQLI_ASSOC)) {
                    $arraylotall[]= array("lot"=>$datalotall1['lot']);
                }
                ...
                $countloc = mysqli_num_rows($dataloc_input);
                ...
                elseif($jumlahdatainputan >= '1'){
                    error_reporting(0);
                    if(in_array_sp($order_nodataloc1, $arrayresult)){
                        if(in_array_sp($lotdataloc1, $arraylot)){  
                            if(in_array_sp($input, $arrayloc)){
                                $hasil = mysqli_query($koneksi, "UPDATE tb_sp_progress SET loc_input='$input' WHERE order_no='$order_nodataloc1' and lot='$lotdataloc1' and pengguna = '$getusername'");
                                $movetable = mysqli_query($koneksi, "INSERT INTO tb_sp_result (tgl,order_no,model,lot,loc_input,case_no,casemark,jam,pengguna,case_plan) SELECT tgl,order_no,model,lot,loc_input,case_no,casemark,jam,pengguna,case_plan FROM tb_sp_progress WHERE pengguna = '$getusername'");
                                $deleted = mysqli_query($koneksi, "DELETE FROM tb_sp_progress WHERE pengguna = '$getusername'"); 
                            }
                        ...
                        elseif(($arraylotall == [ ])){
                            if(in_array_sp($input, $arrayresult)){
...
<button style="background-color:red;top:-300px;position:FIX;left:1500px;height:30px;width:120px;border-radius:0px;color:white;"><a href="logout.php" ><h4 style="color:white;">LOGOUT</h4><br></a></button>
<div class="scroll">
				...
				<div class="container-fluid">
					<div class="row text-muted">
						<div class="col-6 text-left">
							<p class="mb-0">
                            <!-- <a href="#" class="text-muted">&copy; <strong ><b>2022 <i style="color:blue;">Ramandani [19968] </i>,<i style="color:blue;">Rakyan [22202] </i>  YIMM-WJF.</b></strong></a>	  -->
							</p>
						</div>
                        <div>
